...
- **HTTP `/api/messages` 集成测试**：`src/web.ts` 的 `app` 未 export，需先抽 `createAppForTest()` factory，再补 route-level 测试覆盖 `/clear` 拦截。
- **owner 生命周期写操作分散**：`owner_im_id` / `sender_allowlist` / `activation_mode` 的写入点散在 `buildOnNewChat`、`handleOwnerMentionCommand`、`handleReleaseOwnerCommand`、`handleRequireMentionCommand`、`handleAllowCommand`、`handleDisallowCommand`、`POST /reset-owner` 7+ 处，各自手搓 `setRegisteredGroup` + 内存 cache 同步。建议收敛到 owner-management 子系统（`claimOwner` / `releaseOwner` / `setActivationMode`），把不变量集中表达 + 单测覆盖。
